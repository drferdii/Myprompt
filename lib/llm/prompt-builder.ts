...
function buildOperatorPriorities(
  rawIdea: string,
  taskType: TaskType
): string | null {
...
function detectPreferredInstructionLanguage(rawIdea: string): string {
  return INDONESIAN_SIGNAL_PATTERN.test(rawIdea)
    ? 'Bahasa Indonesia'
    : 'English'
}
...
function hasExplicitOutputStructure(rawIdea: string): boolean {
  return (
    EXPLICIT_FORMAT_PATTERN.test(rawIdea) ||
    HEADING_LINE_PATTERN.test(rawIdea)
  )
}

function buildQualityRoutingGuidance(
  rawIdea: string,
  taskType: TaskType
): string {
  const guidance = [
...
export function buildOptimizeSystemPrompt(
  optimizerLane: OptimizeLane = 'DEEP'
): string {
