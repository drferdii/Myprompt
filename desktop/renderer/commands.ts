...
function buildVisiblePromptText(superPrompt: Record<string, unknown>) {
  const fullPrompt =
    typeof superPrompt.fullPrompt === 'string' ? superPrompt.fullPrompt.trim() : ''
...
export function parseConsoleInput(input: string): ParsedConsoleInput {
  ...
  if ((rawCommand === 'drafts' || rawCommand === 'draft') && (subcommand === 'open' || subcommand === 'get')) {
    return {
      kind: 'command',
      command: 'draft.get',
      args: rest.slice(1),
...
export function buildCommandInvocation(
  ...
  if (parsed.command === 'library.save') {
    throw new Error(
      'library.save requires current-run context before it can build a valid payload'
    )
  }
