...
const commandHelpCloseBtn = document.getElementById('commandHelpCloseBtn') as HTMLButtonElement | null
...
const miniStatus = document.getElementById('miniStatus') as HTMLElement | null
...
function renderShellBadges(
  badges: NonNullable<DesktopShellState['badges']> = []
) {
  if (!shellBadges) {
    return
  }
  ...
  shellBadges.innerHTML = badges
    .map(
      (badge) =>
        `<span class="shell-badge tone-${badge.tone}">${badge.label}</span>`
    )
    .join('')
}
    ...
    return {
      ...
      payload: {
        ...
        payload: parsed.args[0]
          ? { category: parsed.args[0].toUpperCase() }
          : {},
      },
    }
  }
...
function buildOptimizePromptText(superPrompt: Record<string, unknown>) {
  const fullPrompt =
    typeof superPrompt.fullPrompt === 'string'
      ? superPrompt.fullPrompt.trim()
      : ''
  ...
  if (
    Array.isArray(superPrompt.constraints) &&
  ...
  if (
    typeof superPrompt.formatSpec === 'string' &&
...
function buildRunRecord(
  ...
  if (mode === 'transform' && isObjectRecord(result)) {
    const transformedPrompt =
      typeof result.transformedPrompt === 'string'
        ? result.transformedPrompt.trim()
        : ''
...
function syncOptimizerLaneModelPresentation() {
  ...
  if (
    modelChip &&
    typeof laneState?.modelChip === 'string' &&
    laneState.modelChip.trim()
  ) {
...
function ensureOptimizeStreamLine(
  container: HTMLElement,
  requestId: string
) {
...
function ensureOptimizeStatusLine(
  container: HTMLElement,
  requestId: string
) {
...
async function executeOptimizeStream(
  ...
  function startDecode(target: string, onComplete: () => void): void {
    if (!activeScrambleLine) { onComplete(); return }
    if (scrambleIntervalId !== null) {
      clearInterval(scrambleIntervalId)
      scrambleIntervalId = null
    }
  ...
  await new Promise<void>(async (resolve, reject) => {
    ...
    try {
      const started =
        (await desktopWindow.sentraDesktop?.invoke?.(
          invocation.channel,
          invocation.payload
        )) ?? null
      ...
      reject(
        error instanceof Error
          ? error
          : new Error('Desktop stream bridge not ready yet.')
      )
    }
  })
}
...
function formatDesktopErrorMessage(error: unknown) {
  const message =
    error instanceof Error
      ? error.message
      : 'Desktop command bridge not ready yet.'
...
function formatBenchmarkReport(result: {
  ...
  for (const item of result.report) {
    ...
    lines.push('', `## ${caseId} · ${lane}`, `- Result: ${pass}`, `- Provider: ${provider}/${model}`)
...
function formatBenchmarkList(benchmarks: DesktopBenchmarkRecord[]) {
  ...
  for (const record of benchmarks) {
    lines.push(
      `- ${record.id} · ${record.title} · lanes=${record.lanes.join(', ')}`
    )
  }
...
function formatDesktopResult(result: unknown): string {
  ...
  const superPrompt = isObjectRecord(result.superPrompt)
    ? result.superPrompt
    : null
  ...
  if (isObjectRecord(result.benchmark) && typeof result.benchmark.id === 'string') {
    return [
      ...
      `- Title: ${
        typeof result.benchmark.title === 'string'
...
async function loadShellState() {
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Unable to load desktop shell state.'
    if (display) {
...
async function execute() {
  ...
  const started = Date.now()
  const pendingLines = [
    appendConsoleLine(display, 'sys', `[WAIT] ${buildPendingLabel()}`),
  ]
  ...
  try {
    ...
    const requestId =
      promptValue !== null && currentMode === 'optimize'
        ? crypto.randomUUID()
        : undefined
    const invocation =
      promptValue !== null
        ...
        : buildCommandInvocation(
            parsed as Extract<ParsedConsoleInput, { kind: 'command' }>
          )
...
for (const button of modeButtons) {
  button.addEventListener('click', () => {
    const nextMode =
      button.dataset.mode === 'optimize' ? 'optimize' : 'transform'
    updateMode(nextMode)
  })
}
...
for (const button of optimizerLaneButtons) {
  button.addEventListener('click', () => {
    const nextLane =
      button.dataset.lane === 'DEEP' ? 'DEEP' : 'INTERACTIVE'
...
copyLastBtn?.addEventListener('click', () => {
  ...
  navigator.clipboard.writeText(lastCopyText).then(() => {
    if (copyLastBtn) {
      ...
      setTimeout(() => { copyLastBtn.textContent = 'COPY' }, 1500)
    }
  })
})
