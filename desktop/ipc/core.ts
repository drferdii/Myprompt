...
import type { DesktopCommandEnvelope } from '../../lib/desktop/contracts'
import type { LLMProviderName } from '../../types'

import { createWorkspaceStore } from './workspace-store'
import {
  listDesktopBenchmarks,
  runDesktopBenchmark,
  saveDesktopBenchmark,
} from './benchmark'
import {
  createDesktopPrompt,
  listDesktopPrompts,
...
const desktopWorkspaceStore = createWorkspaceStore(
  resolveDesktopWorkspaceFilePath()
)
...
function resolveGuestProvider(
  requestedProvider?: unknown
): DesktopGuestProvider {
...
function buildGuestOptimizePayload(payload: any) {
...
function resolveOptimizeLane(payload: any): DesktopOptimizeLane {
...
function buildGuestEvaluatePayload(payload: any) {
...
function buildDesktopCommandAccessError(command: string, error: unknown) {
  ...
  if (
    errorMessage.includes('No API key provided for')
  ) {
...
async function streamOptimizeCommand(
  ...
  try {
    ...
    if (!user) {
      ...
      const modelId = String(optimizePayload.targetLlm ?? '').toLowerCase()
      ...
      if (typeof optimizePayload.provider !== 'string') {
        throw new Error('Optimizer provider is required for desktop streaming.')
      }

      const apiKey = await resolveProviderApiKey(
        user.id,
        optimizePayload.provider as LLMProviderName
      )
      request = {
        ...optimizePayload,
        apiKey,
    ...
    const response = await optimizePromptStreaming(request as any, (delta) => {
...
export async function handleDesktopCommand(
  ...
  try {
    if (command === 'transform:run') {
      return transformPrompt(payload)
    }
    ...
    if (command === 'optimize:run') {
      ...
      if (!user) {
        return optimizePrompt(buildGuestOptimizePayload(payload))
      }
      ...
      const modelId = String(payload.targetLlm ?? '').toLowerCase()
      if (!checkModelAccess(tier, modelId)) {
        return buildModelRestrictedResponse()
      }

      const apiKey = await resolveProviderApiKey(user.id, payload.provider)
      return optimizePrompt({ ...payload, apiKey })
    }
    ...
    if (command === 'evaluate:run') {
      ...
      if (!user) {
        return evaluatePrompt(buildGuestEvaluatePayload(payload))
      }
      ...
      const apiKey = await resolveProviderApiKey(user.id, payload.provider)
      return evaluatePrompt({ ...payload, apiKey })
    }

    if (command === 'library:list') {
      const user = await resolveDesktopUser(accessToken)
      return listDesktopPrompts(user.id, payload)
    }

    if (command === 'library:save') {
      const user = await resolveDesktopUser(accessToken)
      return createDesktopPrompt(user.id, payload ?? {})
    }
      ...
      const draft = await desktopWorkspaceStore.saveDraft({
        id:
          typeof payload?.id === 'string' && payload.id.trim()
            ? payload.id
            : randomUUID(),
        rawInput: typeof payload?.rawInput === 'string' ? payload.rawInput : '',
        optimizedText:
          typeof payload?.optimizedText === 'string'
            ? payload.optimizedText
            : typeof payload?.outputText === 'string'
              ? payload.outputText
              : '',
        sourceMode: resolveWorkspaceSourceMode(payload?.sourceMode),
      })
    ...
    if (command === 'benchmark:save') {
      return saveDesktopBenchmark(desktopWorkspaceStore, payload ?? {})
    }
    ...
    if (command === 'benchmark:run') {
      ...
      const provider = resolveGuestProvider(payload?.provider)
      const apiKey = user
        ? await resolveProviderApiKey(user.id, provider)
        : undefined

      return runDesktopBenchmark(desktopWorkspaceStore, payload ?? {}, {
        provider,
        apiKey,
      })
    }
    ...
    if (command === 'templates:list') {
      return listDesktopTemplates(payload?.category)
    }
    ...
    if (
      ...
      const user = await resolveDesktopUser(accessToken)
      return handleProviderCommand(command, payload, user.id)
    }

    if (command === 'subscription:upgrade') {
      const user = await resolveDesktopUser(accessToken)
      return handleSubscriptionCommand(payload, user)
    }
...
export function registerCoreIpc() {
  ipcMain.handle(
    'desktop:command',
    async (event, envelope: DesktopCommandEnvelope) => {
